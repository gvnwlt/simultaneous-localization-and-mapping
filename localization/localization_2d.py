...
def move(p, motions, p_move):
    # update the grid from sense step with motion 
    m_update = [[0 for row in range(len(p[0]))] for col in range(len(p))]
    for i in range(len(p)):
        for j in range(len(p[0])):
            m_update[i][j] = (p_move * p[(i - motions[0]) % len(p)][(j - motions[1]) % len(p[0])]) + (p[i][j] * (1-p_move))
    # return updated grid 
    return m_update
